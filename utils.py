...
class Transd2Ind:
    ...
    def __init__(self, dpr_data, keep_ratio):
        ...
        idx_unlabeled = list(set(idx_all) - set(idx_train) - set(idx_val) - set(idx_test))
        idx_train = list(idx_train)+idx_unlabeled
        ...
        print('#edges in adj_train:', int(self.adj_train.sum()))
...
def result_record_acc_pro(ptb_rate, res, result_path, method_name, args):
    result_path_file = result_path+method_name+'.csv'

    if not os.path.exists(result_path):
        os.makedirs(result_path)

    mAP_mean = res.mean(0)[0]*100
    mAP_std = res.std(0)[0]*100
    mAPs=[f"{mAP_mean:.2f}+{mAP_std:.2f}"]

    with open(result_path_file ,'a+',newline='') as f:
        writer = csv.writer(f)
        content = ["ptb_rate", f"{ptb_rate}",
                "knn", f"{args.knn}",
                "warm_up", f"{args.warm_up}",
                "stru_epoch", f"{args.stru_epoch}",
                "acc", f"{mAPs[0]}"]
        writer.writerow(content)


class Transd2Ind_attack:
    # transductive setting to inductive setting

    def __init__(self, dpr_data, keep_ratio, **kwargs):
        idx_train, idx_val, idx_test = list(dpr_data.idx_train), list(dpr_data.idx_val), list(dpr_data.idx_test)
        adj, features, labels = dpr_data.adj, dpr_data.features, dpr_data.labels

        if 'attack' in kwargs:
            attack = kwargs['attack']
        if 'ptb_rate' in kwargs:
            ptb_rate = kwargs['ptb_rate']
        if 'method' in kwargs:
            method = kwargs['method']
        #attack
        if not (attack == 'no' or ptb_rate==0):
            dataset_str_attack = './attacked_adj/'+ kwargs['dataset']+f'/adj_{attack}_{ptb_rate}.npz'
            adj = sp.load_npz(dataset_str_attack)

        self.nclass = labels.max()+1
        self.adj_full, self.feat_full, self.labels_full = adj, features, labels
        idx_all = list(range(len(labels)))
        idx_unlabeled = list(set(idx_all) - set(idx_train) - set(idx_val) - set(idx_test))
        idx_train = idx_train+idx_unlabeled
        ...
        self.adj_train = adj[np.ix_(idx_train, idx_train)]
        self.adj_val = adj[np.ix_(idx_train+idx_val, idx_train+idx_val)]
        self.adj_test = adj[np.ix_(idx_train+idx_test, idx_train+idx_test)]
        ...
        self.feat_train = features[idx_train]
        self.feat_val = features[idx_train+idx_val]
        self.feat_test = features[idx_train+idx_test]

        self.idx_val = np.arange(len(idx_train+idx_val))[-len(idx_val):]
        self.idx_test = np.arange(len(idx_train+idx_test))[-len(idx_test):]
